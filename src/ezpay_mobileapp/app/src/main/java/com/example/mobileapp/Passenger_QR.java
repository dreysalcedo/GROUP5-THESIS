...
public class Passenger_QR extends AppCompatActivity {

    //Initialize Variables
    String startpoint, endpoint, type;
    String initial,destination,cname,distance,fare;
    ...
    TextView showdistance,showdestination;
    TextView showfare, fare_mobile_number,back;
    Button compute, pay;
    ImageView qr_output;
    private String generate_id,total_fare,mobile_number,init_location,init_destination,init_status,total_distance;
    String time = getTime();
    String date = getDate();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_passenger__q_r);

        //Assign Variables

        getInput_startlocation = findViewById(R.id.txt_qr_initial);
        getInput_endpoint = findViewById(R.id.txt_qr_destination);

        showdistance = findViewById(R.id.tv_qr_totaldistance);
        showfare = findViewById(R.id.tv_qr_totalfare);
        pay = findViewById(R.id.btn_pay);
        fare_mobile_number = findViewById(R.id.tv_mb);
        compute =  findViewById(R.id.btn_calc_compute);

        get_passengertype =  findViewById(R.id.spinner_qr_passengertype);
        get_transportation =  findViewById(R.id.spinner_qr_transpotype);

        back = findViewById(R.id.back);

        //pass value from Dashboard Activity
        Intent i = getIntent();
        fare_mobile_number.setText(i.getStringExtra("mobile_number")); //tv is the TextView
        ...
        back.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(Passenger_QR.this, Passenger_Dashboard.class);
...
//set the spinners adapter to the previously created one.
        ...
        compute.setOnClickListener(v -> {
            ...
            if (!transportation_mode.equals("") && !passenger.equals("") && !showdistance.equals("")) {
                ...
                if (transportation_mode.equals("Taxi")) {
                    int flagdown = 40;
                    double per_km = 13.50;
                    double traveleddistance = Double.parseDouble(String.valueOf(showdistance.getText()));
                    double fare_calculation = flagdown + (traveleddistance * per_km);
                    DecimalFormat df = new DecimalFormat(("#.##"));
                    fare_calculation = Double.valueOf(df.format(fare_calculation));
                ...
                //if jeep and passenger = regular
                    ...
                    double per_km = 1.24; // succeeding kilometers = adittional PHP 1.24 per km
                    ...
                    double per_km = 1.75; //  PHP 1.75 per km
                    double init_km = 8.75; // first 5 = 8.75
                    double traveleddistance = Double.parseDouble(String.valueOf(showdistance.getText()));
                    //if distance is less than or equals 5
                    if (traveleddistance <= 5) {
                        //pay PHP 7.20 for discounted fare
                ...
                }//if aircon bus and passenger is discounted 20%
                else if (transportation_mode.equals("Air Conditioned Bus") && passenger.equals("Discounted")) {
                    double per_km = 1.40; //  PHP 1.4 per km
                    double init_km = 7.0; // first 5 = 8.75
                    double traveleddistance = Double.parseDouble(String.valueOf(showdistance.getText()));
                    //if distance is less than or equals 5
                    if (traveleddistance <= 5) {
                        //pay PHP 7.20 for discounted fare
        ...
        pay.setOnClickListener(v -> {
            ...
            HttpsTrustManager.allowAllSSL();
            //Get Text
            init_location = getInput_startlocation.getText().toString();
            init_destination = getInput_endpoint.getText().toString();
            total_distance = showdistance.getText().toString();
            mobile_number = fare_mobile_number.getText().toString();
            total_fare = showfare.getText().toString();
            generate_id = fare_mobile_number.getText() + date + time;//transaction id\
            init_status = "Pending";
            //check if empty string
            if (!init_location.equals("") && !init_destination.equals("") && !mobile_number.equals("") && !total_distance.equals("") && !total_fare.equals("") &&
                    !generate_id.equals("") && !init_status.equals("")) {
                Handler handler = new Handler(Looper.getMainLooper());
                handler.post(() -> {
                    //Starting Write and Read topUpData with URL
                    //Creating array for parameters
                    String[] field = new String[7];
                    field[0] = "transaction_id";
                    field[1] = "fare";
                    field[2] = "mobile_number";
                    field[3] = "initial_location";
                    field[4] = "destination";
                    field[5] = "status";
                    field[6] = "distance";
                    ...
                    data[3] = init_location;
                    data[4] = init_destination;
                    data[5] = init_status;
                    data[6] = total_distance;

                    PutData putData = new PutData("https://sdffea.com/Thesis/save.php", "POST", field, data);
                    if (putData.startPut()) {
                        if (putData.onComplete()) {
                            ...
                            //check if what register.php throws
                                ...
                                Intent intent = new Intent(this, Passenger_Generate_QR.class);
                                intent.putExtra ( "transaction id", generate_id);
                                startActivity(intent);
                                finish();
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 100 && resultCode == RESULT_OK) {
            ...
            if (flag >= 2) {
                ...
                //Calculate distance
                Double parseDistance = (haversine(places_lat1, places_long1, places_lat2, places_long2));
                DecimalFormat df = new DecimalFormat(("#.##"));
                parseDistance = Double.valueOf(df.format(parseDistance));
                showdistance.setText(parseDistance.toString());
